...
export const messageHandlers = {
  ...
  async notification_action(clientId, message) {
    ...
    const payload = {
      ...
      ok: !!result.ok,
      error: result.ok ? null : (result.error || 'CALLBACK_FAILED'),
      status: result.status || (result.ok ? 'callback_succeeded' : 'internal_error'),
      http_status: result.httpStatus ?? null
    };

    try {
      connMgr.sendToClient(clientId, payload);
    } catch (_) {}
  },
