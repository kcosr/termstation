import { BASIC_AUTH } from './config.mjs';
...
export class ApiClient {
  ...
  async deleteSession(sessionId) {
    // Need raw response for status-only semantics; do not parse body on success
    const url = joinUrl(this.baseUrl, `/sessions/${encodeURIComponent(sessionId)}`);
    if (this.debug) console.error(`[agents][debug] DELETE ${url}`);
    const res = await fetch(url, { method: 'DELETE', headers: { 'Authorization': BASIC_AUTH } });
